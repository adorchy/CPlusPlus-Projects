...
void FBullCowGame::PrintIntro() {
	...
	cout << " *  |-,--- |              |-(--)-|  * " << endl;
	cout << "    ^      ^              ^  ''  ^ " << endl;
	cout << "Can you guess the " << GetWordLength();
	cout << " letters isogram I'm thinking of?\n";
	cout << GetMaxTries() << " tries availables\n\n";
}
...
// optimized function using TMap to check is a word is an Isogram
...
void FBullCowGame::PlayGame() {
	string Guess = "";
	while (GetCurrentTry() <= GetMaxTries() && IsGameWon() == false){
		EGuessStatut GuessStatut = None;
		do {
			Guess = GetGuess(); // get the guess
			GuessStatut = CheckGuessValidity(Guess); // check guess validity
		} while (GuessStatut != Ok); // loop continually until the user gives a valid guess
		SubmitGuess(Guess); //incriment Bulls ands Cows
	}
